	...
	<div class="col-md-10 " >
		...
		<div class="panel panel-default  "  style="padding:2px;margin-bottom:1px;" >
			...
			?></b>
				<?php
				if (!empty($stock_info)){
					echo '<br>'. $stock_info["name"].' stock in use';
				}
				?>
			</div>
				...
				<?php 
					...
					/*
						...
						*/
						echo '<tr>';
							echo '<td>';
								if (isset($clinic_prescription_info["Status"])){
									echo 'Status : <b>'.$clinic_prescription_info["Status"].'</b>';
								}
							echo '</td>';						
							echo '<td>';
							...
							echo '</td>';
							echo '<td>';							
								if (isset($clinic_prescription_info["PrescribeDate"])){
									echo 'Prescribe Date : <b>'.$clinic_prescription_info["PrescribeDate"].'</b>';
									
								}
							echo '</td>';
						echo '</tr>';	
					echo '</table><br>';	
					//print_r($clinic_prescription_info);
					if (isset($prescribe_items_list)){
						if ($clinic_prescription_info["Status"] =="Pending"){
							echo '<form id="drug_form" method="POST" action="'.site_url("pharmacy/save_clinic_dispense").'" >';
						}
						echo '<input id="clinic_prescription_id" name="clinic_prescription_id" type="hidden" value="'.$clinic_prescription_info["clinic_prescription_id"].'" >';
						echo '<input id="drug_stock_id" name="drug_stock_id" type="hidden" value="'.$stock_info["drug_stock_id"].'" >';
						echo '<table class="table table-condensed table-hover" style="margin-bottom:0px;">';
						...
						//print_r($prescribe_items_list);
							for($i=0; $i < count($prescribe_items_list);++$i){
								echo '<tr>';
									...
									if ($prescribe_items_list[$i]["Status"] =="Pending"){
										echo '<input id="'.$prescribe_items_list[$i]["clinic_prescribe_item_id"].'" name="'.$prescribe_items_list[$i]["clinic_prescribe_item_id"].'" type="number" min=0 step=1 max=130 value="'.$d_count.'">';		
										echo '<input id="_'.$prescribe_items_list[$i]["clinic_prescribe_item_id"].'" name="_'.$prescribe_items_list[$i]["clinic_prescribe_item_id"].'" type="hidden" value="'.$prescribe_items_list[$i]["DRGID"].'">';		
									}
									else{
						...
						echo '</table>';
							...
							echo '<a  href="'.site_url("pharmacy/show_list/".$clinic_prescription_info["Dept"]).'">Back to list</a>';
							echo '</center>';
							if ($clinic_prescription_info["Status"] =="Pending"){
								echo '</form>';
							}
